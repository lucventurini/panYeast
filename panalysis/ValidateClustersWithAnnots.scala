...
object ValidateClustersWithAnnots extends ActionObject {
  ...
  override def main(args: Array[String]) = {
    ...
    val scores = clusterScores(clustering, annots)
...
//    }
  ...
  def clusterScores(clustering: Clustering, annots: Annotations): Array[(Int,Int,Int,Int,Int,Double)] = {
    clustering.intClusters.map{ c =>
      ...
      (c.id, nGenes.toInt, annotatedGenes.size, c.toProtein(clustering.protMap).cluster.map(p => p.taxa).distinct.length, clusterAnnotations.keys.size, score)
    }
  }
