...
rule orthofinder_all:
  ...
  params:
    rule_outdir = "%s" % __ORTHOFINDER_OUTDIR__,
    orthofinder_params = tconfig["orthofinder_blast_params"] + tconfig["orthofinder_mcl_params"]
  conda: "%s/conda_envs/orthofinder.yaml" % __PIPELINE_COMPONENTS__
  shell: """
    orthofinder-dl.py -a 10 -t {threads} -f {params.rule_outdir}/input {params.orthofinder_params} -og --constOut
  """
...
rule orthofinder:
  input:
    protmap    = rules.orthofinder_all.output.sequenceids,
    mci_output = rules.orthofinder_all.output.mci_output,
    statistics = rules.orthofinder_all.output.statistics
  output:
